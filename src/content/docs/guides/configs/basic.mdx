...
ShokaX 支持使用 HyC 控制台以图形界面的方式修改配置项，无需修改配置文件
...
# 浏览器打开提示的官方控制台，即------^^^^^^^^^^^^^^^^^
# 在控制台中输入16位认证码，随后进入控制台
```

进入控制台后，在左侧导航栏内选择“博客配置”，进入对应页面后选择“主题配置”，即可开始使用交互式配置：

![控制台](./basic/console.png)

修改完成后点击保存配置，HyC 会自动将配置文件写入硬盘，并立刻生效

如果你更喜欢修改文件，也可以查看下面的章节：
...
| `locale`   | `"zh-CN"` \| `"zh-TW"` \| `"ja"` \| `"en"` | `"zh-CN"` | 网站界面语言。                                 |
